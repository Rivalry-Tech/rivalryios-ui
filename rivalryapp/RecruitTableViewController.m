...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (indexPath.section == contentSection)
    {
        ...
        if (selectedCell.accessoryType == UITableViewCellAccessoryNone)
        {
            dispatch_async(dispatch_get_main_queue(), ^
            {
                [UIAlertView showWithTitle:@"Content Subscription" message:@"You are subscribing to receive notifications for articles from DawgPost. This is a free service. Would you like to continue?" cancelButtonTitle:@"NO" otherButtonTitles:@[@"YES"] tapBlock:^(UIAlertView *alertView, NSInteger buttonIndex)
                {
                    if (buttonIndex == 1)
                    {
                        PFObject *provider = [contentProviders objectAtIndex:indexPath.row];
